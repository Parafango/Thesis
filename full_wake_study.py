        ...
        for i_t, TI in enumerate(TI_arr):
            ...
            for i_y, yaw_angle in enumerate(yaw_arr):
                ...
                for i_dd, dd in enumerate(DD_arr):
                    ...
                    V_new = flatten_wind_field(Z, V_mat, alfa, Vm, href)
                    V_squared, Y_squared, Z_squared, y_c, z_c = get_U_squared_no_z0(V_new, Y, Z)
                    u_max = abs(np.amin(V_squared))

                    if u_max<0.38:
                        V_squared = np.multiply(V_squared, 0.38/u_max)
                        popt, perr, R_flat, V_flat = gauss_interp(Y_squared, Z_squared, y_c, z_c, V_squared)
                        popt[0] = popt[0] * u_max/0.38
                        V_flat = np.divide(V_flat, 0.38/u_max)
                    else:
                        popt, perr, R_flat, V_flat = gauss_interp(Y_squared, Z_squared, y_c, z_c, V_squared)

                    z_shape = np.shape(V_squared)[0]
                    y_shape = np.shape(V_squared)[1]
                    ...
                    dev = np.abs(np.add(interp_values, -abs(V_flat)))
...
filepath = Path('C:/Users/randr/Desktop/full_wake_circular_noz0.csv')
...
# --------------------------------------------elliptical wake--------------------------------------------------------------
# definition of storing numpy matrix
gauss_parameters = np.zeros((np.shape(der_arr)[0] * np.shape(Vm_arr)[0] * np.shape(TI_arr)[0] * np.shape(yaw_arr)[0] *
                             np.shape(offset_arr)[0] * np.shape(DD_arr)[0], 20))
        ...
        for i_t, TI in enumerate(TI_arr):
            ...
            for i_y, yaw_angle in enumerate(yaw_arr):
                ...
                for i_dd, dd in enumerate(DD_arr):
                    ...
                    if u_max < 0.38:
                        V_squared = np.multiply(V_squared, 0.38 / u_max)
                        popt, perr, yz_arr, V_arr = gauss_interp_2d(Y_squared, Z_squared, y_c, z_c, V_squared)
                        popt[0] = popt[0] * u_max / 0.38
                        u_arr = np.divide(V_arr, 0.38 / u_max)
                    else:
                        popt, perr, yz_arr, V_arr = gauss_interp_2d(Y_squared, Z_squared, y_c, z_c, V_squared)
                    ...
                    dev = np.abs(np.add(interp_values, -abs(V_arr)))
                    err_max = round(np.amax(dev), 3)
                    err_mean = round(np.mean(dev), 3)
                    stdv = round(np.std(dev), 3)
                    dev_rel = np.divide(dev, Vm / 100)
                    ...
                    for i_o, offset in enumerate(offset_arr):
                        ...
                        popti = np.array(
                            [der, Vm, TI, yaw_angle, dd, offset, A, y_c_d, z_c_d, sigmay, sigmaz, Vm, err_max, err_mean,
                             stdv, err_rel_max, err_rel_mean, stdv_rel, y_shape, z_shape])
                        gauss_parameters[i_d * m_d + i_v * m_v + i_t * m_t + i_y * m_y + i_dd * m_dd + i_o, :] = popti

everything = pd.DataFrame(gauss_parameters,
                          columns=['der', 'Vm', 'TI', 'yaw', 'DD', 'offset', 'peak', 'yc_d', 'zc_d', 'sigmay', 'sigmaz',
                                   'Vm', 'e_max', 'e_mean', 'e_stdv', 'e_rel_max', 'e_rel_mean', 'e_rel_stdv',
                                   'y_shape', 'z_shape'])

filepath = Path('C:/Users/randr/Desktop/full_wake_elliptical_noz0.csv')
